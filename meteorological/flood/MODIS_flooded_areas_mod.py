...
import os, zipfile, arcpy, datetime, string,shutil,urllib2,sys
...
def download(url):
        ...
        try:
            ...
            timed_out = True
            print "error type 1"
            timed_out_list.append(file_name)
        except socket.timedout as e:
            timed_out = True
            print "error type 2"
            print type(e)
            timed_out_list.append(file_name)
        ...
        try:
            ...
            timed_out = True
            print "timed out in writing"
            timed_out_list.append(file_name)
        #print "bookmark 4"
        print "timed_out_list: " + str(timed_out_list)
        return file_name, timed_out
...
def _downloadAll(tile):
        ...
        for link in soup.findAll('a'):
            ...
            if daytilezip != "mailto:kettner@colorado.edu":
        ...
        for i in daysAvailableAndNotCollected:
                if i[:7] >= oldest_yearday:
                    newfiles.append(i)
        print "newfiles: " + str(newfiles)
        newfiles_to_return = newfiles
        for filename in newfiles:
                file = open(os.path.join(tempPath,filename),"wb")
                newfile = os.path.join(tempPath,filename)
                localcount = localcount + 1
                url = remote_folder + filename
                file_name, timed_out = download(url)
                mess1 = 'DOWNLOADED %s ' % filename
                mess2  = ' - Zipfile n. %s' % localcount
                mess3 = ' in the tile n. %s' % count
                mess4 = ' (of %s)' % totTiles
                ...
                file.close()
                timeout_count = 0
                while timed_out == True and timeout_count < 5:
                    print "timed out time n." + str(timeout_count)
                    file_name, timed_out = download(url)
                if timed_out == True:
                    print "removing " + filename + " from list"
                    newfiles_to_return.remove(filename)
...
logfile = r"C:\Users\andrea.amparore\Desktop\Scheduled_tasks\MODIS_SurfaceWater\Modis_floods_logfile.txt"
...
if len(listofunzippedshapes) > 0:
        ...
        _removeOld()
        print ""
        final_message = 'Mission accomplished, new data processed and appended.'
        print final_message
        #arcpy.AddMessage('Mission accomplished, man! Go and check the output here: ' + output_location)
else:
    final_message = 'Nothing new available, no actions taken'
    print final_message

with open(logfile,'a') as f:
    f.write("\n" + str(today.isoformat()) + " at " + hour + ": " + final_message)
