...
class GeocodeEMDAT(object):
    ...
    def __init__(self, paese, hazard, dir_out, gaul_shp,gaul_dir):
        self.paese = paese
        self.hazard = hazard
        ...
        self.gaul_shp = gaul_shp
        self.gaul_dir = gaul_dir
        self.totali = 0
        self.successo = 0
        self.insuccesso = 0
        self.gaul_file = None
    ...
    def extract_country_shp(self):
            ...
            print self.gaul_file
            print self.gaul_dir

            inShapefile = str(self.gaul_dir)
...
class CreateGeocodedShp(object):
    ...
    def __init__(self, paese, hazard,dir_out):
        self.paese = paese
        self.hazard = hazard
        self.dir_out = dir_out
...
paese = pycountry.countries.get(name = 'Indonesia')
iso = paese.alpha3
...
# df_emdat_paese_divisi = df_valori_letti[['disaster_no','location']]
df_emdat_paese_divisi = df_emdat_paese_divisi.dropna(axis=0,how='any')
# locazioni_su_colonna = df_emdat_paese_divisi['location'].str.split(",").apply(pd.Series, 1).stack()
...
# df_emdat_paese_finale = df_emdat_paese_divisi.join(locazioni_su_colonna.apply(lambda x: pd.Series(x.split(':'))))
...
#FASE GEOCODING
app = wx.App()
dialog_out = wx.DirDialog(None, "Choose a directory for output:",style=wx.DD_DEFAULT_STYLE | wx.DD_NEW_DIR_BUTTON)
if dialog_out.ShowModal() == wx.ID_OK:
    dir_out = dialog_out.GetPath()
dialog_out.Destroy()

openFileDialogSHP = wx.FileDialog(None, "Choose GAUL shapefile:", "", "",
                                       "shp files (*.shp)|*.shp", wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)

if openFileDialogSHP.ShowModal() == wx.ID_CANCEL:
    pass

gaul_file = openFileDialogSHP.Filename
gaul_dir = openFileDialogSHP.GetPath()
openFileDialogSHP.Destroy()
...
geocodiamo = GeocodeEMDAT(nome_paese, 'Storm', dir_out,gaul_file,gaul_dir)
...
shapiamo = CreateGeocodedShp(nome_paese,'Storm',dir_out)
