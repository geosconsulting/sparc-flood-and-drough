...
class AppECMWF:
    ...
    def __init__(self, finestra):
        ...
        lista_anni_correnti = list(range(1973, in_che_anno_siamo-1))
    ...
    def calcola_mean_from_historical_forecasts(self):
        ...
        mese_inizio = self.now.month
        giorno_inizio = self.now.day
        salto = self.days_check.get()

        range_anni_scelti = range(int(anno_minimo), int(anno_massimo)+1)
        date_per_creazione_files = calculate_time_window_date.controlla_date(anno_minimo, mese_inizio,giorno_inizio, salto)
